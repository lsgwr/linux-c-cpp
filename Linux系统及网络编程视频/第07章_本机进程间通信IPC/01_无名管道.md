...
+ 1）pipefd[0]：里面放的是读管道的**读**文件描述符
+ 2）pipefd[1]：里面放的是写管道的**写**文件描述符
...
void print_error(char *str) {
    fprintf(stderr, "File %s, Line %d, Function %s error\n",__FILE__, __LINE__, __func__); // 打印错误位置
    perror(str); // 打印错误信息
    exit(-1); // 退出程序
}
...
{
    int ret = 0;
    // 每个进程中的文件描述符(fd)池都是相互独立地,最多都可以有1024个，范围为0~1023
    int pipefd[2] = {0}; // 定义无名管道的读写管道.在父子进程中的pipefd数组是相互独立地，fd值相同但不是一个，一定要区分好
    
    ret = pipe(pipefd);
    if(ret == -1){
        print_error("pipe fail");
    }
    printf("读标识符(管道0)：%d, 写标识符(管道1)：%d\n", pipefd[0], pipefd[1]);
    ...
    if(ret > 0){
        // 父进程
        close(pipefd[0]); // 用不到读管道，先关闭
        char buf_w[] = "hello";
        write(pipefd[1], buf_w, sizeof(buf_w)); // 写管道写入数据
        sleep(1); // 让子进程先结束防止出现僵尸进程
    }else if(ret == 0){
        // 子进程
        close(pipefd[1]); // 用不到写管道，先关闭
        char buf_r[30] = {}; // 读取数据的数组
        bzero(buf_r, sizeof(buf_r));
        read(pipefd[0], buf_r, sizeof(buf_r)); // 读管道读入数据
        printf("子进程中从父进程读取到的数据: %s\n", buf_r);
    }
    return 0;
}
```
...
+ （b）什么时候会产生在这个信号？  
  ...
  **所以父子进程必须保证一读一写，否则就会出现SIGPIPE异常中断信号**

  由于这个信号的默认动作是终止，所以收到这个信号的进程会被终止，如果你不想被终止的话，你可以忽略、捕获、或者屏蔽这个信号(下面代码注释掉的的`signal(SIGPIPE, SIG_IGN);`和` close(pipefd[0]);`)
  ...
  int main(void)
  {
      ...
      if(ret > 0){
          // 父进程
          // signal(SIGPIPE, SIG_IGN); // 父进程忽略SIGPIPE信号，可以使得程序不异常(ps -al可以看到父子进程俩)
          close(pipefd[0]); // 用不到读管道，先关闭
          while(1){ // 保证父子进程不断通信，方便用ps -al观察
              char buf_w[] = "hello";
              write(pipefd[1], buf_w, sizeof(buf_w)); // 写管道写入数据
              sleep(1); // 让子进程先结束防止出现僵尸进程
          }
          ...
          // 子进程
          close(pipefd[1]); // 用不到写管道，先关闭
          // close(pipefd[0]); // 子进程读写都关闭，会产生SIGPIPE信号，如果不处理(捕获或者忽略地话，ps -al只能看到父进程自己)
          while(1){ // 保证父子进程不断通信，方便用ps -al观察
              char buf_r[30] = {}; // 读取数据的数组
              bzero(buf_r, sizeof(buf_r));
              read(pipefd[0], buf_r, sizeof(buf_r)); // 读管道读入数据
...
使用**两个无名管道**，每个管道负责一个方向的通信。

![两个无名管道实现双向通信](https://i.loli.net/2019/04/04/5ca5f68c8c699.jpg)

见https://edu.51cto.com//center/course/lesson/index?id=262736 的3:29

简化后图示如下：

![两个无名管道实现双向通信2](https://i.loli.net/2019/04/04/5ca5f8309eb5c.jpg)
见https://edu.51cto.com//center/course/lesson/index?id=262736 的4:43

总结：
+ 父进程：fd[1]和fd1[0]打开，fd[0]和fd1[1]关闭
+ 子进程：fd[0]和fd1[1]打开，fd[1]和fd1[0]关闭
...
int main(void)
{
    ...
    // 每个进程中的文件描述符(fd)池都是相互独立地,最多都可以有1024个，范围为0~1023
    int pipefd[2] = {0}; // 定义无名管道的读写管道.在父子进程中的pipefd数组是相互独立地，fd值相同但不是一个，一定要区分好
    int pipefd1[2] = {0}; // 定义无名管道的读写管道.在父子进程中的pipefd数组是相互独立地，fd值相同但不是一个，一定要区分好
    ...
    ret = pipe(pipefd1);
    if(ret == -1){
        print_error("pipe1 fail");
    }
    
    printf("管道0读标识符：%d, 写标识符：%d\n", pipefd[0], pipefd[1]);
    printf("管道1读标识符：%d, 写标识符：%d\n", pipefd1[0], pipefd1[1]);
    
    ret = fork(); // 创建子进程
    if(ret > 0){
        // 父进程
        close(pipefd[0]); // 用不到管道0的读管道，先关闭
        close(pipefd1[1]); // 用不到管道1写管道，先关闭
        while(1){ // 保证父子进程不断通信，方便用ps -al观察
            char buf_w[] = "hello";
            char buf_r[30] = {}; // 读取数据的数组
            bzero(buf_r, sizeof(buf_r));
            write(pipefd[1], buf_w, sizeof(buf_w)); // 写管道写入数据
            sleep(1); // 让子进程先结束防止出现僵尸进程
            read(pipefd1[0], buf_r, sizeof(buf_r)); // 读管道读入数据
            printf("父进程中从子进程读取到的数据: %s\n", buf_r); // 应该是world
        }
    }else if(ret == 0){
        // 子进程
        close(pipefd[1]); // 用不到管道0的写管道，先关闭
        close(pipefd1[0]); // 用不到管道1的读管道，先关闭
        while(1){ // 保证父子进程不断通信，方便用ps -al观察
            char buf_w[] = "world";
            char buf_r[30] = {}; // 读取数据的数组
            bzero(buf_r, sizeof(buf_r));
            write(pipefd1[1], buf_w, sizeof(buf_w)); // 向父进程写入数据
            sleep(1);
            read(pipefd[0], buf_r, sizeof(buf_r)); // 读管道读入数据
            printf("子进程中从父进程读取到的数据: %s\n", buf_r); // 应该是hello
        }
...
结果如下：hello 和 world 交替打印,写操作一定要在读前面，要不会出现读阻塞

```shell
管道0读标识符(管道0)：3, 写标识符(管道1)：4
管道1读标识符(管道0)：5, 写标识符(管道1)：6
父进程中从子进程读取到的数据: world
子进程中从父进程读取到的数据: hello
父进程中从子进程读取到的数据: world
子进程中从父进程读取到的数据: hello
父进程中从子进程读取到的数据: world
子进程中从父进程读取到的数据: hello

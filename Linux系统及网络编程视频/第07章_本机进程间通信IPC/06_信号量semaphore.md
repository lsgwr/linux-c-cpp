...
比如当多个进程同时共享向文件里面写数据时，同样会出现和共享写“共享内存”相同的情况. 图：
...
+ （c）参数说明  
  ...
  + cmd：控制选项  
    ...
    IPC_STAT、IPC_SET、IPC_RMID是一样的。但是SETVAL确属于进程信号量所独有的选项。
    ...
    + cmd为SETVAL：第四个参数应该设置为一个int的值，用于初始化信号量  
      ...
      成员：val：存放用于初始化信号量的值; buf：存放struct semid_ds结构体变量的地址

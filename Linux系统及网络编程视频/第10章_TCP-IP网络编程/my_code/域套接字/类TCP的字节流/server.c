...
int main(int argc, char const *argv[])
{
    ...
    server_addr.sun_family = AF_UNIX; // 使用是IPV4 TCP/IP协议族的ip地址(32位)
    strcpy(server_addr.sun_path, SOCK_FILE); // 设置域套接字文件的路径
    ...
    /* 第4步：调用accept 网络API，域套接字直接用文件来通信, 所以第2、3个参数都是不用设置具体值*/
    cfd = accept(skfd, NULL, 0); 

...
static int configuration(int *port, char *path)
{
	...
	while(fgets(buf, 50, fp) != NULL){ /* 读取文件每一行的内容 */
		...
		/* 端口号的配置格式为“prot: 8000”，注意“：”后面有一个空格，找到“：”的位置 */
...
int error_page(int sock_fd)
{
	...
	if(my_write(sock_fd, "Content-Type: text/html\r\n\r\n", strlen ("Content-Type: text/html\r\n\r\n")) == -1)
		return -1;
	if(my_write(sock_fd, "<html><body> the file dose not exsit </body></html>", strlen("<html><body> the file dose not exsit </body></html>")) == -1) /* 输出html内容 */
		return -1;
	return 0;
}
/* 向客户端输出需要的页面
...
int write_page(int cfd,int fd, char * path)
{
	...
	if(my_write(cfd, "\r\n\r\n", 4) == -1) /* 添加协议结尾，最后多出一个“\r\n” */
		return -1; 
	while ((n = read(fd, buf, MAX_LINE)) > 0) /* 传输文件内容 */

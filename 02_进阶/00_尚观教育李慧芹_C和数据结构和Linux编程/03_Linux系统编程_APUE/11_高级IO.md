...
epoll() 不是一个函数，它在 man 手册的第 7 章里，它是 Linux 为我们提供的“加强版 poll()”，既然是加强版，那么一定有超越 poll() 的地方，下面就聊一聊 epoll()。 

在使用 poll() 的时候用户需要管理一个 struct pollfd 结构体或它的结构体数组，epoll() 则使内核为我们管理了这个结构体数组，我们只需要通过 epoll_create() 返回的标识引用这个结构体即可。

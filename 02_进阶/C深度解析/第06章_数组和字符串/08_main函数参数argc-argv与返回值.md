...
```c
char *buf[] = {"how", "are", "you"};  //字符串指针数组，ps : 类似二维数组的指针形式
```
相当于如下3个字符数组的合并，所以上面的`char *buf[]`相当于`字符数组的数组`即`字符串数组`：  
...
作为字符串常量放在了.rodata中，buf[]中的每个元素只是用来存放每个字符串的字符串指针(三个单词各自第一个字母的首地址)  

"how", "are", "you"为字符串常量，各自代表的是第0个元素空间的指针（字符串指针），字符串的每个字节空间的类型为char，那么空间指针的指针类型则为`char *`，自然第0个字符所在空间的指针也为`char *`，所以存放字符串指针的数组也为`char *`  
